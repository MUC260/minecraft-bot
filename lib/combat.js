...
function toolForBlock (blockName, items) {
  ...
  const pickaxeLike = oreLike || [
    'stone', 'cobblestone', 'deepslate', 'cobbled_deepslate', 'netherrack',
    'blackstone', 'basalt', 'tuff', 'calcite', 'obsidian', 'crying_obsidian'
  ].includes(n) || n.endsWith('_stone') || n.endsWith('_bricks') || n.endsWith('_concrete') || n.endsWith('_terracotta')
  const shovelLike = ['dirt', 'grass_block', 'sand', 'gravel', 'clay', 'soul_sand'].includes(n)
  const kinds = logLike ? ['axe'] : pickaxeLike ? ['pickaxe'] : shovelLike ? ['shovel'] : []
