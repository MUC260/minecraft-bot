...
const { equipBestToolForBlock } = require('../lib/tool-selector')
...
const handlers = {
  ...
  dig: async (bot, args, ctx) => {
    ...
    const tool = await equipBestToolForBlock(bot, block)
    await raceWithAbort(bot.dig(block, true), ctx, () => {
      if (typeof bot.stopDigging === 'function') bot.stopDigging()
    })
    return `挖掘 ${block.name}${tool ? `，使用 ${tool.name}` : ''}`
  },
  ...
  collect: async (bot, args, ctx) => {
    ...
    if (block) {
      ...
      return '\u91c7\u96c6 ' + block.name + (tool ? `，使用 ${tool.name}` : '')
    }
