...
class Brain {
  constructor (agent, config) {
    ...
    this.awakeningEnabled = config.awakeningEnabled === true
    this.planIntervalMs = this.awakeningEnabled ? Math.max(120000, Number(config.awakeningIntervalMs) || 120000) : 0
    this.planning = false
  ...
  setAwakening (enabled) {
    this.awakeningEnabled = enabled === true || enabled === 'true' || enabled === 1
    this.planIntervalMs = this.awakeningEnabled ? Math.max(120000, Number(this.config.awakeningIntervalMs) || 120000) : 0
    if (this.planTimer) clearTimeout(this.planTimer)
    this.planTimer = null
    if (this.awakeningEnabled && this.running) this._startPlanLoop()
    if (!this.awakeningEnabled) this.longTermPlan = null
    return this.awakeningEnabled
  }

  setGoal (goal) {
    if (goal && String(goal).trim()) this.goal = String(goal).trim()
    if (this.awakeningEnabled) this.nudgePlan(250)
    return this.goal
  }
  ...
  start () {
    ...
    this._kick(50)
    if (this.awakeningEnabled) this._startPlanLoop()
  }
  ...
  nudgePlan (ms = 0) {
    if (!this.running || !this.awakeningEnabled || this.planning) return
    if (this.planTimer) clearTimeout(this.planTimer)
    this.planTimer = setTimeout(() => this._makeLongTermPlan(), Math.max(0, ms))
  }

  // 3 分钟规划循环：每次规划完成后安排下一次
  _startPlanLoop () {
    if (!this.running || !this.awakeningEnabled) return
    if (this.planTimer) clearTimeout(this.planTimer)
  ...
  async _makeLongTermPlan () {
    if (!this.running || !this.awakeningEnabled || this.planning || !this.agent.connected) return
  ...
  _userPayload (snapshot) {
    ...
    if (this.awakeningEnabled && this.longTermPlan && Date.now() - this.longTermPlanAt < this.planIntervalMs * 2) {
