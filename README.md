...
任何玩家在游戏聊天框中对机器人说话即可唤醒 AI 对话。**唤醒词默认是 `@机器人名字`**（机器人叫什么名字就 @ 什么），也可以在游戏内修改。

| 你在游戏里说 | 机器人做什么 |
| --- | --- |
| `@AIBot 去附近砍树` | AI 分析指令，调用工具执行动作 |
| `@AIBot 帮我找钻石` | AI 规划路线、导航并采集 |
| `@AIBot 建造一个房子` | AI 规划建材、逐步搭建 |

AI 对话由 `aiMembers` 白名单控制，只有**授权玩家**才能使用。主人（owner）默认可用，普通玩家需要管理员添加授权。
...
| 你在游戏里说 | 机器人做什么 |
| --- | --- |
| `!ai list` | 查看当前唤醒词和已授权成员 |
| `!ai add 玩家名` | 授权某玩家使用 @ai |
| `!ai remove 玩家名` | 移除某玩家的授权 |
| `!ai clear` | 清空所有授权 |
| `!ai setword @自定义词` | 自定义唤醒词（如改为 `@AI`） |
| `!ai help` | 显示帮助 |

配置修改后自动持久化到 `config.json`，无需手动编辑文件。

> **权限说明**：主人（ownerName）默认可用所有指令；`aiMembers` 白名单仅控制 `@ai` 对话权限，不影响主人指令。
